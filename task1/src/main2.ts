...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const numberInputEl = document.getElementById('numberInput') as HTMLInputElement | null;
    ...
    calculateBtn.addEventListener('click', () => {
        const userInput: string = numberInputEl.value;
        ...
        if (typeof result === 'string') {
            resultOutput.innerHTML = `<span class="error">${result}</span>`;
        } else {
            resultOutput.innerHTML = `<span class="success">Square root of ${userInput} is ${result.toFixed(6)}</span>`;
        }
    });
});
